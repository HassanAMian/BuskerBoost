require 'spec_helper'

describe "Status pages" do

  subject { page }

  let(:user) { FactoryGirl.create(:user) }
  before { sign_in user }

  describe "status creation" do
    before { visit root_path }

    describe "with invalid information" do

      it "should not create a status" do
        expect { click_button "Share" }.not_to change(Status, :count)
      end

      describe "error messages" do
        before { click_button "Share" }
        it { should have_content('error') }
      end
    end

    describe "with valid information" do

      before { fill_in 'status_content', with: "Lorem ipsum" }
      it "should create a stauts" do
        expect { click_button "Share" }.to change(Status, :count).by(1)
      end
    end
  end

  describe "status destruction" do
    before { FactoryGirl.create(:status, user: user) }

    describe "as correct user" do
      before { visit root_path }

      it "should delete a status" do
        expect { click_link "delete" }.to change(Status, :count).by(-1)
      end
    end
  end
end